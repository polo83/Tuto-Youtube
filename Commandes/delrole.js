module.exports.run = (client, message, args) => {
    ...
    if (!message.guild.member(message.author.id).hasPermission('MANAGE_ROLE')) { return message.channel.send('Vous n\'avez pas la permission `gérer les roles` !'); }
    if (!message.guild.member(client.user.id).hasPermission('MANAGE_ROLE')) { return message.channel.send('Je n\'ai pas la permission `gérer les roles` !'); }
